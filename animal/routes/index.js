...
/*  // 上传图片
router.post('/adopt_article', (req, res) => {
...
/*动物领养页面 */
/*router.post('/adopt_article', function (req, res) {
...
/*宠物信息 */
/*router.get('/adopt_article',function(req,res){
})*/

// 添加动物信息
router.post('/adopt_article', (req, res) => {
  const product = req.body
  ArticleModel.create(product)
    .then(product => {
 ...
 */
router.get('/adoptInfo', function (req, res) {
  // 查询
  ArticleModel.find({},function(err,artcles){
      if (err) {
        console.error(err)
        return
      }
      res.send({code: 0, data: artcles})
  })
})
